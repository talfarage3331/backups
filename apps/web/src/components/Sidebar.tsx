import { LayoutDashboard, GitBranch, Settings, Shield, LogOut, ScanSearch, Server } from 'lucide-react';
...
interface SidebarProps {
  ...
  piiAlertCount?: number;
}

export default function Sidebar({ activePage, onNavigate, activePipelinesCount, piiAlertCount = 0 }: SidebarProps) {
  ...
  return (
    <aside className="sidebar">
      ...
      <div className="sidebar-logo">
        ...
        <span className="sidebar-logo-name">EnvShield</span>
      </div>
      ...
      <nav className="sidebar-nav">
        ...
        <button
          id="nav-schema"
          className={`sidebar-link ${activePage === 'schema' ? 'active' : ''}`}
          onClick={() => onNavigate('schema')}
        >
          <ScanSearch size={17} />
          Schema Explorer
          {piiAlertCount > 0 && (
            <span className="sidebar-badge" style={{ background: 'var(--accent-amber)' }}>
              {piiAlertCount}
            </span>
          )}
        </button>
        ...
        <button
          id="nav-environments"
          className={`sidebar-link ${activePage === 'environments' ? 'active' : ''}`}
